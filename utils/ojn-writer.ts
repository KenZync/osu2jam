...
export const createOJN = async (
	...
	for (const measure in parsedPackage) {
		for (const channel in parsedPackage[measure]) {
			...
			for (const event in parsedPackage[measure][channel].Events) {
				...
				let type = parsedPackage[measure][channel].Events[event].type

				if (type === 'note' || type === 'ogg') {
					let value = 0
					if (type === 'ogg') {
						value = 1
					}
					...
					//VALUE
					dataView.setInt16(cursor, value, true)
					cursor += 2
					//VolumePan
	...
	const fileName = 'o2ma' + songData.songid + '.ojn'
